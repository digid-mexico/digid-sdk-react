...
| `E422 ... Unsupported GitHub Actions source repository visibility: "private". Only public source repositories are supported when publishing with provenance` | `--provenance` exige que el repositorio de GitHub sea **público**; con uno privado npm rechaza la firma | Hacer público el repositorio, o quitar `--provenance` del workflow (se pierde la certificación de origen y hay que corregir los enlaces del README, que apuntan a un repo que el cliente no podría abrir) |
